...
func (e PaymentEdges) PayerOrErr() (*Customer, error) {
	if e.loadedTypes[0] {
		if e.Payer == nil {
...
func (*Payment) scanValues(columns []string) ([]interface{}, error) {
	...
	for i := range columns {
		...
		case payment.FieldID, payment.FieldCustomerID, payment.FieldStaffID, payment.FieldRentalID:
...
// QueryPayer queries the "payer" edge of the Payment entity.
func (pa *Payment) QueryPayer() *CustomerQuery {
	return (&PaymentClient{config: pa.config}).QueryPayer(pa)
}
