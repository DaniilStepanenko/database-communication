...
func (pq *PaymentQuery) QueryPayer() *CustomerQuery {
...
func (pq *PaymentQuery) WithPayer(opts ...func(*CustomerQuery)) *PaymentQuery {
	query := &CustomerQuery{config: pq.config}
...
//		GroupBy(payment.FieldCustomerID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Payment.Query().
//		Select(payment.FieldCustomerID).
//		Scan(ctx, &v)
...
func (pq *PaymentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Payment, error) {
	...
	if query := pq.withPayer; query != nil {
		if err := pq.loadPayer(ctx, query, nodes, nil,
			func(n *Payment, e *Customer) { n.Edges.Payer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PaymentQuery) loadPayer(ctx context.Context, query *CustomerQuery, nodes []*Payment, init func(*Payment), assign func(*Payment, *Customer)) error {
