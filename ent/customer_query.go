...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withPayments; query != nil {
		...
		query.Where(predicate.Payment(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.PaymentsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.customer_payments
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_payments" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_payments" returned %v for node %v`, *fk, n.ID)
			}
